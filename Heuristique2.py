...
def check(values,liste,digit):
    ...
    return values
#applique l'heuristique locked candidates recurcivement a chaque fois que des candidats sont trouvé
def locked_candidates(values,valeur_par_defaut):
    ...
    return values
#applique l'heuristique hidden singles pour reduire la taille des candidats
...
#applique les heuristique hidden singles et locked candidates recursivement pour  eliminer le plus de candidat que possible
#appele l'algorithme du brute force de norvig apres l'aplication des deux heuristique
def heuristique2(values):
    valeur_par_defaut= default_values(values)
    values=hidden_singles(values,valeur_par_defaut)
    values=locked_candidates(values,valeur_par_defaut)
